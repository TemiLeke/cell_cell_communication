...
#projection_data=NULL;saveFiles=T;benchmark_mode=T
...
.extra_sconline.argFn=function(runIndx,HVG_list,min_cluster_size,pseudocell_selection_method,saveDir,prop.n.neighbors = 4,ncores=7,conservativeMapping=F,oldMapping=F,MGIsymbol=F,includeHuman,includeMouse,DE_supportingFractionThr,DE_n.adaptiveKernel,DE_nPropIter,Leng200=F,uniformZscore=F,dist_zscore_gamma,dist_zscore_norm,dist_zscore_nbinom,regularize,geoMean=F,prefix=NULL,newRun=F,inputDf=NULL,internal_pseudocell_count,pseudocell_size,external_DE_path=NULL,external_DE_name=NULL,do.liger=NULL,singleton.method=singleton.method,include.singletons=T,colNormalize=T){
  ...
  .saveDir=.mySaveDirMaker(paste0(saveDir,"/",prefix),nPCs = .nPCs,cov=.covariates)
  .saveDirGlobal=paste0(saveDir,"/",prefix,"-Global")
  
  
  argList=list(commonExpressed=.dfSetting$commonExpressed[runIndx],HVG_list=HVG_list,min_cluster_size=min_cluster_size,pseudocell_selection_method=pseudocell_selection_method,prop.n.neighbors = prop.n.neighbors,includeHuman=.includeHuman,includeMouse=.includeMouse,conservativeMapping=conservativeMapping,oldMapping=oldMapping,MGIsymbol=MGIsymbol,nPCs=.dfSetting$nPCs[runIndx],HVG_list=.dfSetting$HVG_count[runIndx],HVG_method="vst",include.singletons=include.singletons,colNormalize=colNormalize,covariates=.covariates,indScaling=.dfSetting$indScaling[runIndx],saveDir=.saveDir,saveDirGlobal=.saveDirGlobal,ncores=ncores,excludeHighExp=.removeHighExp,breakHammond=.breakHammond,UMI_cor_thr=.UMI_cor_thr,onlyRankBased=.dfSetting$onlyRankBased[runIndx],varScore.thr=.dfSetting$varScore.thr[runIndx],do.liger=.dfSetting$do.liger[runIndx],allGenesFraction=.dfSetting$allGenesFraction[runIndx],uniformZscore=uniformZscore,DE_supportingFractionThr=DE_supportingFractionThr,DE_n.adaptiveKernel=DE_n.adaptiveKernel,DE_nPropIter=DE_nPropIter,dist_zscore_gamma=dist_zscore_gamma,dist_zscore_norm=dist_zscore_norm,dist_zscore_nbinom=dist_zscore_nbinom,regularize=regularize,geoMean=geoMean,prefix=prefix,internal_pseudocell_count=internal_pseudocell_count,pseudocell_size=pseudocell_size,external_DE_path=external_DE_path,external_DE_name=external_DE_name,singleton.method=singleton.method)
...
.sconline.FilterGenesByExpr <- function(inputExpData, raw_sce, threshold=0.10, filter_from='raw', TMMnorm = TRUE, quantile.norm = FALSE, prior.count = 0.5){
  ...
  # Ensure further inclusion of genes with meaningful expression dynamics/variability across samples
...
#inputExpData=tmp_ps;covariates=setdiff(covList,"anno_cov");randomEffect=rand_var;bkg_genes=tmp_bkg_genes;quantile.norm=quantile_norm;prior.count=1
#DEmethod="Trend";normalization="CPM";quantile.norm=F;VST_fitType="parametric";prior.count=1;include.malat1.as.covariate=F;dc.object=NULL;dream_ncores=4
...
.sconline.Pseudobulk10=function(inputExpData,embeddings,pseudobulk_split_col,min_dataset_size=4,organism='Human'){
  ...
  pseudo_res$QC_Gene_total_count=apply(counts(pseudo_res),2,sum)
  pseudo_res$QC_Gene_unique_count=apply(counts(pseudo_res),2,function(x) sum(x>0))

  pseudo_res$QC_MT.pct = .extraMitoPctFn(inputData = pseudo_res,organism = organism)
...
.sconline.GSEA.AUCellfn=function(expData,gs_path,gs_list=NULL,min.gs.size=10,aucell_maxrank_fraction=0.1,max.gs.size=500,ncores=10){
  require(AUCell)
  ...
  if(length(gs_elsivier)>0){
    ...
    cells_rankings <- AUCell_buildRankings(as.matrix(counts(expData)),nCores = ncores)
...
#prefix=ds_name;saveDir = file.path(file.path(main_dir,istudy,"data_arranged",ds_name));pseudocell_size=50;n.prop=4;nPCs=30;HVG_list=HVG_count;indScaling=T;ncores=min(parallel::detectCores(), 8, na.rm=T);input_highly_var_genes=NULL;newRun=F;min_ds_size=50
#min_cluster_size=20;n.prop=4;pseudocell_selection_method="kmeans";pseudocell_size=200;input_highly_var_genes=NULL;newRun=F;min_ds_size=50;Leng200=F;include.singletons=T;colNormalize=T;singleton.method="fast"

.sconline.arg_creator=function(prefix,min_cluster_size=20,n.prop=4,saveDir,nPCs,HVG_list=NULL,indScaling=T,ncores=min(parallel::detectCores(), 8, na.rm=T),pseudocell_selection_method="kmeans",pseudocell_size=200,input_highly_var_genes=NULL,newRun=F,min_ds_size=50,Leng200=F,include.singletons=T,colNormalize=T,singleton.method="fast"){
  ...
  dfSettings=rbind(dfSettings,data.frame(commonExpressed=T,
                                         nPCs=nPCs,
                                         HVG_list=HVG_list,covariates="",indScaling=indScaling,removeHighExp=F,breakHammond=T,UMI_cor_thr=0.3,onlyRankBased=F,varScore.thr=5000,allGenesFraction=0.5,do.liger=F,stringsAsFactors = F))
  inputExpData=data
...
#inputExpData,argList,batch_variable="anno_batch",organism="unknown",addAnno=F,hierarchical_mode=F,set_HVG_count_thr=2000
#inputExpData = input_data;argList = argList;batch_variable = "anno_batch";organism="Human";addAnno=F;hierarchical_mode=F;set_HVG_count_thr=2000
.sconline.pca=function(inputExpData,argList,batch_variable="anno_batch",organism="unknown",addAnno=F,hierarchical_mode=F,set_HVG_count_thr=2000,benchmark_mode=F){
  #library(rliger)
  library(Seurat)
  ...
  if(run_check){
    ...
    if(sum(names(data)=="varFeatures")==0){
    ...
    if(length(argList$HVG_list)==0&(!is.null(set_HVG_count_thr))){
      ...
      argList$HVG_list=tmp[set_HVG_count_thr,2]+1
    }
    ...
    if(length(data$varFeatures[data$varFeatures[,2]>=argList$HVG_list,1])<500){
      if(is.null(set_HVG_count_thr)){
        ...
        tmp=data$varFeatures
        tmp=tmp[order(tmp[,2],decreasing = T),]
        argList$HVG_list=tmp[set_HVG_count_thr,2]+0.999
        
      }
    }
    cat('Scaling the data and PCA\n')
    res=.myPCAfn(data,argList = argList,saveFiles = (!hierarchical_mode),benchmark_mode=benchmark_mode)
    if(!hierarchical_mode){
      res="Done!"
    }
  } else {
...
.sconline.runPropagation=function(argList,inputEmbeddings=NULL,inputPhenoData=NULL,inputExpData=NULL,input_UMAP_embedding=NULL,organism,batch_variable="anno_batch",run_harmony=F,addAnno=F,addClusteringModule=F,umap.method='umap-learn',extendedMode=F,L2Norm=T,mergePseudocells=T,generateUMAP=T,merging_strength=0.3,sig1_thr=3,pct2_thr=0.3,pct_diff_thr=0.2){
  ...
  if(length(argList$HVG_list)>1){
    stop("Only one value should be provided for HVG_list")
  } else if(is.null(argList$HVG_list)){
    stop("value for HVG_list is missing")
  }
...
.sconline.create_seurat=function(argList,inputExpData=NULL,n_clusters=NULL,inputEmbeddings=NULL){
  ...
  if(length(argList$HVG_list)>1){
    stop("Only one value for the HVG_list should be provided")
  } else if(is.null(argList$HVG_list)){
    stop("value for HVG_list is missing!")
  }
  ...
  if(!is.null(n_clusters)){
    ...
    if(class(c_assignments)==class("")){
      Idents(inputExpData)=c_assignments
    } else {
...
.sconline.HVGselection=function(argList,inputExpData,batch_variable="anno_batch",run_harmony=F,L2Norm=T,benchmark_mode=F){
  ...
  cat('-- Scaling the data and PCA\n')
  pca_data=.myPCAfn(data,argList = argList,saveFiles=F,benchmark_mode=benchmark_mode)
...
.sconline.subset_purityAnalysis=function(argList,sl_cells=NULL,inputPCAdata=NULL,batch_variable="anno_batch",collapse_datasets=T,minCellCountThr=4,analysis_seed=1,extendedMode=F,cluster_count=NULL){
  ...
  if(is.null(inputPCAdata)){
    ...
    data=.extra_sconline.exp_creatorFn(argList=argList,inputExpData=inputExpData,batch_variable=batch_variable,organism = "unknown",addAnno=F)
    argList2=argList
    argList2$HVG_list=min(max(round(length(data$data)/2),1),argList$HVG_list)
    ...
    cat('-- Scaling the data and PCA\n')
    pca_data=.myPCAfn(data,argList = argList2,saveFiles=F,benchmark_mode=F)
...
.extra_hierarchical_run=function(argList,inputEmbeddings=NULL,inputPhenoData=NULL,organism,batch_variable,inputExpData,L2Norm,mergePseudocells,merging_strength,sig1_thr,pct2_thr,pct_diff_thr,run_harmony=F,index=NULL,dataset_size_thr=20000,min_cluster_size=500,input_res_embeddings=NULL,input_res_prop=NULL){
  ...
  if(nrow(res_prop$prop_mat)==1){
    ...
    #input_res=res_prop;input_pca_centroids=res_embeddings$pca_centroid;input_embeddings=res_embeddings$harmony_embeddings;forgiveness_factor=1;min_cluster_size=500;prune_clusters=F
  ...
  if(length(unique(clust_res[,"cluster"]))>1){
    ...
    for(iclust in unique(clust_res[,"cluster"])){
      ...
      if(length(sl_cells)/ncol(res_prop$prop_mat)>=0.95){
        ...
        tmp_res_prop$prop_mat=tmp_res_prop$prop_mat[row.names(tmp_res_prop$prop_mat) %in% tmp_res_embeddings$sl_pseudo$cluster,]
        tmp_res_prop$prop_mat=tmp_res_prop$prop_mat[rowSums(tmp_res_prop$prop_mat)>0.5,]
        ...
        tmp_res_prop$res_meta$meta_z=tmp_res_prop$res_meta$meta_z[row.names(tmp_res_prop$res_meta$meta_z) %in% row.names(tmp_res_prop$prop_mat),]
        tmp_res_prop$res_meta$med_pct.1=tmp_res_prop$res_meta$med_pct.1[row.names(tmp_res_prop$res_meta$med_pct.1) %in% row.names(tmp_res_prop$prop_mat),]
        tmp_res_prop$res_meta$logFC=tmp_res_prop$res_meta$logFC[row.names(tmp_res_prop$res_meta$logFC) %in% row.names(tmp_res_prop$prop_mat),]
      }
      ...
      if(class(tmp_clust_res)==class(T)){
        qsave(list(sl_cells=sl_cells,pheno=inputPhenoData,exp=inputExpData,argList=argList,res_prop=res_prop,res_embeddings=res_embeddings),file="~/torm.qs")
        stop("Error!")
        
        #tmp=qread("~/torm.qs")
        #sl_cells=tmp$sl_cells;inputPhenoData=tmp$pheno;inputExpData=tmp$exp;res_prop=tmp$res_prop;res_embeddings=tmp$res_embeddings
      }
